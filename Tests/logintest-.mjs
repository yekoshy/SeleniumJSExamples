...
async function testPositive(username, password, successMsg) {
    let title = await login.getTitle('#login > h2');
    assert.equal(title, 'Test login', 'Wrong title!');
    await login.setUsername(username);
    await login.setPassword(password);
    await login.clickItem('#submit');
    let successFlag = await login.isSuccessDisplayed();
    assert.equal(successFlag, true, "Success message isn't displayed!");
    let message = await login.getSuccessMsg();
    assert.equal(message, successMsg, 'Wrong success message!');
}
